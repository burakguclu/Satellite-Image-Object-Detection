...
for bolge in ayarlar["bolgeler"]:
    ...
    cikti_dosyasi = f"{bolge['isim']}_ULTIMATE_HIBRIT_COFFEE_SHOP.png"

    if not os.path.exists(zemin_dosyasi):
        print(f"Atlanıyor: {zemin_dosyasi} bulunamadı.")
        continue
        
    # 1. İSTATİSTİK SÖZLÜĞÜNE KAFE VE SKORLARI EKLEYELİM
    istatistik = {
        ...
        "Kafe (Rakip) Sayısı": 0,         # YENİ
        "Tahmini Refah Skoru": 0.0,       # YENİ
        "Şube Açma Fırsat Endeksi": 0.0   # YENİ
    }
    ...
    # 2. OVERPASS SORGUSUNA KAFELERİ (amenity=cafe) EKLEYELİM
    overpass_sorgusu = f"""
    ...
    (
      ...
      nwr["amenity"="cafe"]({sorgu_guney},{sorgu_bati},{sorgu_kuzey},{sorgu_dogu});
    );
    out tags geom;
    """
    ...
    if cevap.status_code == 200:
        ...
        print(f"     HATA! OSM Verisi Çekilemedi. Sunucu Kodu: {cevap.status_code}")
        print(f"     Nedeni: {cevap.text[:200]}")
        elemanlar = []
    ...
    # 3. KAFELERİ SAYMA İŞLEMİ (Çizim döngülerinin altına ekle)
    for e in elemanlar:
        if e.get('tags', {}).get('amenity') == 'cafe':
            istatistik["Kafe (Rakip) Sayısı"] += 1
            
            # İstersen kafeleri haritada göstermek için kahverengi yuvarlaklar çizebilirsin:
            # (Eğer 'node' ise lat/lon doğrudan gelir, 'way' ise geometry içindedir)
            lat = e.get('lat') or (e.get('geometry')[0]['lat'] if e.get('geometry') else None)
            lon = e.get('lon') or (e.get('geometry')[0]['lon'] if e.get('geometry') else None)
            if lat and lon:
                x, y = piksele_cevir(lat, lon)
                cizici.ellipse([x-5, y-5, x+5, y+5], fill=(139, 69, 19, 255), outline=(255, 255, 255, 255)) # Kahverengi Kafe Noktası

    # 4. DÖNGÜ SONUNDA ENDEKSLERİ HESAPLAMA (Tüm binalar sayıldıktan sonra)
    # Ortalama ev büyüklüğü (Eğer hiç AI bina yoksa 0 al)
    ortalama_bina_m2 = 0
    if istatistik["Yapay Zeka (AI) Bina Sayısı"] > 0:
        ortalama_bina_m2 = istatistik["Toplam İnşaat Alanı (m2)"] / istatistik["Yapay Zeka (AI) Bina Sayısı"]

    # Refah Skoru (Katsayıları makalendeki teze göre değiştirebilirsin)
    refah_skoru = (istatistik["Havuz Sayısı"] * 50) + (ortalama_bina_m2 * 2) + (istatistik["Toplam Yeşil Alan (m2)"] / 1000)
    istatistik["Tahmini Refah Skoru"] = round(refah_skoru, 2)

    # Şube Açma Fırsat Endeksi (White Space Index) = Refah / Rakipler
    # Kafe sayısına 1 ekliyoruz ki bölgede hiç kafe yoksa (Acarkent gibi) formül sıfıra bölme hatası vermesin.
    firsat_endeksi = refah_skoru / (istatistik["Kafe (Rakip) Sayısı"] + 1)
    istatistik["Şube Açma Fırsat Endeksi"] = round(firsat_endeksi, 2)
